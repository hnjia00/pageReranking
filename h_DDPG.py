...
class meta_controller_DDPG(object):
    ...
    def _build_a(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Actor', reuse=reuse, custom_getter=custom_getter):
            ...
            return tf.clip_by_value(a, 0, 100000)
...
class controller_DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound):
        self.memory = np.zeros((c_MEMORY_CAPACITY, s_dim * 2 + a_dim + 1), dtype=np.float32)
        self.pointer = 0
        self.sess = tf.Session()
    ...
    def _build_a(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('c_Actor', reuse=reuse, custom_getter=custom_getter):
            net = tf.layers.dense(s, 64, activation=tf.nn.relu, name='c_l1', trainable=trainable)
            a = tf.layers.dense(net, self.a_dim, activation=tf.nn.relu, name='c_a', trainable=trainable)
            # return tf.multiply(a, self.a_bound, name='c_scaled_a')
            return tf.clip_by_value(a, 0, 100000)
